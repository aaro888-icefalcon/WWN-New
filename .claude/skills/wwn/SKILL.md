...
3. **WORLDGEN — define the world (player-in-the-loop, scope-scaled).** Run `references/gm/worldgen.md`:
   - Ask the player the **scope** (a single region ▸ a few neighboring nations ▸ a kingdom ▸ a whole continent), tone, magic level, and the **world source** — one of four: **default Latter Earth** (the Gyre / the Atlas) ▸ a **fresh world** (`--fresh`) ▸ a **provided setting** (ingest the player's own into `setting-canon.md`) ▸ **guided/modified Latter Earth** (Latter Earth with the player setting one region's themes/gameplay). **Generate factions here too** (`--campaign campaign` → `campaign/factions.md`); faction moves then fire every scene at bookkeeping. Full setup options: `SETUP-GUIDE.md`.
